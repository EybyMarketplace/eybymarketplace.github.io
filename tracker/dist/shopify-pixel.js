const IdGenerator = {
  ...
  analytics.subscribe('customer_logged_in', (event) => {
    trackEvent('fp_customer_logged_in', {
  ...
  analytics.subscribe('customer_logged_out', (event) => {
    trackEvent('fp_customer_logged_out', {
      customer_id: event.customerId,
      timestamp: event.timestamp
    });
  });
  ...
  analytics.subscribe('page_viewed', (event) => {
    ...
    if (pathname.includes('/products/')) pageType = 'product';
    else if (pathname.includes('/collections/')) pageType = 'collection';
    else if (pathname === '/cart') pageType = 'cart';
    else if (pathname.includes('/search')) pageType = 'search';
    else if (pathname === '/') pageType = 'home';
    else if (pathname.includes('/pages/')) pageType = 'page';
    else if (pathname.includes('/blogs/')) pageType = 'blog';
    else if (pathname.includes('/account')) pageType = 'account';
  
    trackEvent('fp_page_view', {
      page_type: pageType,
  ...
  analytics.subscribe('product_viewed', (event) => {
    trackEvent('fp_product_view', {
      product: extractProductData(event.data.productVariant.product),
      variant: extractVariantData(event.data.productVariant),
      timestamp: event.timestamp
    });
  });
  
  analytics.subscribe('product_added_to_cart', (event) => {
    trackEvent('fp_add_to_cart', {
      product: extractProductData(event.data.cartLine.merchandise.product),
      variant: extractVariantData(event.data.cartLine.merchandise),
  ...
  analytics.subscribe('product_removed_from_cart', (event) => {
    trackEvent('fp_remove_from_cart', {
      product: extractProductData(event.data.cartLine.merchandise.product),
      variant: extractVariantData(event.data.cartLine.merchandise),
      quantity: event.data.cartLine.quantity,
      line_price: parseFloat(event.data.cartLine.merchandise.price.amount) * event.data.cartLine.quantity,
  ...
  analytics.subscribe('product_variant_changed', (event) => {
    trackEvent('fp_variant_changed', {
      from_variant: extractVariantData(event.data.previousProductVariant),
      to_variant: extractVariantData(event.data.productVariant),
      product: extractProductData(event.data.productVariant.product),
      timestamp: event.timestamp
    });
  });
  ...
  analytics.subscribe('cart_viewed', (event) => {
    trackEvent('fp_cart_view', {
      ...
      items: extractLineItems(event.data.cart.lines),
      timestamp: event.timestamp
    });
  });
  
  analytics.subscribe('cart_updated', (event) => {
    trackEvent('fp_cart_updated', {
      cart_token: event.data.cart.id,
      total_price: parseFloat(event.data.cart.cost.totalAmount.amount),
      currency: event.data.cart.cost.totalAmount.currencyCode,
      item_count: event.data.cart.lines.length,
  ...
  analytics.subscribe('checkout_started', (event) => {
    trackEvent('fp_checkout_started', {
  ...
  analytics.subscribe('checkout_address_info_submitted', (event) => {
    trackEvent('fp_checkout_address_submitted', {
      checkout_token: event.data.checkout.token,
      shipping_address: event.data.checkout.shippingAddress,
      billing_address: event.data.checkout.billingAddress,
      timestamp: event.timestamp
  ...
  analytics.subscribe('checkout_contact_info_submitted', (event) => {
    trackEvent('fp_checkout_contact_submitted', {
      checkout_token: event.data.checkout.token,
      email: event.data.checkout.email,
      phone: event.data.checkout.phone,
      timestamp: event.timestamp
  ...
  analytics.subscribe('checkout_shipping_info_submitted', (event) => {
    trackEvent('fp_checkout_shipping_submitted', {
      checkout_token: event.data.checkout.token,
  ...
  analytics.subscribe('payment_info_submitted', (event) => {
    trackEvent('fp_payment_info_submitted', {
      checkout_token: event.data.checkout.token,
  ...
  console.log('🚀 Commerce Tracker Pixel carregado com sucesso!');